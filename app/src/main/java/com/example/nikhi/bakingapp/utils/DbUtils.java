package com.example.nikhi.bakingapp.utils;
...
import com.example.nikhi.bakingapp.data.source.local.db.IngredientPersistenceContract.IngredientEntry;
import com.example.nikhi.bakingapp.data.source.local.db.RecipePersistenceContract.RecipeEntry;
...
public class DbUtils {
  ...
  public static ContentValues stepToContentValues(@NonNull Step step, int recipeId) {
    ...
    cv.put(StepPersistenceContract.StepEntry.COLUMN_RECIPE_ID, recipeId);
    cv.put(StepPersistenceContract.StepEntry.COLUMN_STEP_ID, step.id());
    cv.put(StepPersistenceContract.StepEntry.COLUMN_SHORT_DESC, step.shortDescription());
    cv.put(StepPersistenceContract.StepEntry.COLUMN_DESC, step.description());
    cv.put(StepPersistenceContract.StepEntry.COLUMN_VIDEO_URL, step.videoURL());
    cv.put(StepPersistenceContract.StepEntry.COLUMN_THUMB_URL, step.thumbnailURL());
  ...
  public static List<Step> stepsFromCursor(@NonNull Cursor cursor) {
    ...
    if (cursor.getCount() > 0) {
      ...
      while (cursor.moveToNext()) {
        ...
        int stepId = cursor.getInt(cursor.getColumnIndexOrThrow(StepPersistenceContract.StepEntry.COLUMN_STEP_ID));
        String sDesc = cursor.getString(cursor.getColumnIndexOrThrow(StepPersistenceContract.StepEntry.COLUMN_SHORT_DESC));
        String desc = cursor.getString(cursor.getColumnIndexOrThrow(StepPersistenceContract.StepEntry.COLUMN_DESC));
        String video = cursor.getString(cursor.getColumnIndexOrThrow(StepPersistenceContract.StepEntry.COLUMN_VIDEO_URL));
        String thumb = cursor.getString(cursor.getColumnIndexOrThrow(StepPersistenceContract.StepEntry.COLUMN_THUMB_URL));
